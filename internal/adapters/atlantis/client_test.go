...
func TestCommandRejectsUnsafeSelectorsBeforeHTTP(t *testing.T) {
	...
	cases := map[string][]domain.RootSelector{
		...
		"directory glob":  {{Directory: "prod/*"}},
		"recursive glob":  {{Directory: "prod/**/api"}},
		"single wildcard": {{Directory: "prod/?pi"}},
		"character class": {{Directory: "prod/[ad]pi"}},
		"cleaned glob":    {{Directory: "prod/*/../api"}},
